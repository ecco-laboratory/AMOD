...
% models and valence, arousal and their interaction, using the OASIS database
...
%%
% get OASIS images
% read number of OASIS images based on runs into list
...
for s = 1:length(subject)
    ...
    % get predicted activations from encoding models
...
regression_object.fullpath = 'voxelwise_correlations_OASIS_valence_amygdala.nii';
...
table(threshold(ttest(regression_object),.005,'UNC'));
regression_object.fullpath = 'voxelwise_correlations_OASIS_arousal_amygdala.nii';
regression_object.write;
%%
regression_object=masked_dat;
regression_object.removed_voxels = ~any(excluded_voxels==0)';
...
table(threshold(ttest(regression_object),.005,'UNC'));
regression_object.fullpath = 'voxelwise_correlations_OASIS_valence_arousal_interaction_amygdala.nii';
regression_object.write;